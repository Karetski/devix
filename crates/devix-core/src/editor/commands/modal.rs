...
use crate::{HandleCtx, Outcome, Pane, Rect, RenderCtx};
...
impl Pane for PalettePane {
    ...
    /// from the app's render module via `crate::render_palette`,
...
/// Thin wrapper over `crate::palette::format_chord` so callers can
/// pass a `Chord` directly. Lives here because `Chord` is a commands type.
pub fn format_chord(chord: Chord) -> String {
    crate::widgets::palette::format_chord(chord.code, chord.mods)
}
